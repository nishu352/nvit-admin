    ...
    <main className="p-4 sm:p-8 space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 dark:border-slate-900 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400 font-semibold mt-0.5">Maintain corporate classification indices, map categories, and merge duplicates</p>
        </div>
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          <button
            onClick={() => setShowMergeModal(true)}
            className="h-10 px-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 text-slate-700 dark:text-slate-200 text-xs font-bold transition-all flex items-center gap-2 cursor-pointer shadow-xs"
          >
            <Merge className="w-4 h-4 text-purple-600 dark:text-purple-400" />
        ...
        <div className="flex items-center gap-3">
          {selectedIds.length > 0 && (
            <button
              ...
              className="h-10 px-4 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 hover:border-amber-300 dark:hover:border-amber-500/40 text-amber-700 dark:text-amber-400 text-xs font-bold flex items-center gap-2 cursor-pointer transition-colors"
            >
          ...
          <button
            ...
            className="h-10 px-4 rounded-xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-850 hover:bg-slate-50 dark:hover:bg-slate-900 text-slate-700 dark:text-slate-300 text-xs font-bold flex items-center gap-2 cursor-pointer shadow-xs"
          >
                  ...
                  companies.map((c: any) => (
                    <tr key={c.id} className="hover:bg-slate-50 dark:hover:bg-slate-850/40 transition-colors">
                      <td className="py-4.5 px-6">
                        <button onClick={() => toggleSelect(c.id)} className="text-slate-400 hover:text-slate-600 dark:hover:text-white cursor-pointer">
                      ...
                      <td className="py-4.5 px-6 font-black text-slate-900 dark:text-white">{c.name}</td>
                          <td className="py-4.5 px-6 font-mono text-[10px] text-slate-500 dark:text-slate-450">{c.cin || "UNLISTED"}</td>
                          <td className="py-4.5 px-6">
                            <div className="flex flex-wrap gap-1.5 max-w-sm">
                              {c.bankCategories.length === 0 ? (
                                <span className="text-[9px] font-bold text-slate-400 dark:text-slate-600">No Mappings</span>
                              ) : (
          ...
          <AnimatePresence>
            {showAddModal && (
              <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                ...
                >
                  <h2 className="text-lg font-black text-slate-900 dark:text-white">{editingCompany ? "Modify Company Index" : "Register Company Index"}</h2>
                  <form onSubmit={handleCreateOrUpdate} className="space-y-4">
                    <div className="space-y-1">
                      <label className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">Company Name *</label>
                      <input
                        type="text"
                        required
                        value={name}
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">Corporate CIN (Optional)</label>
                      <input
                        type="text"
                        value={cin}
              ...
              <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-lg font-black text-slate-900 dark:text-white">Merge Duplicate Indexing</h2>
                  </div>
                  <p className="text-[11px] text-slate-600 dark:text-slate-400 font-medium">Merge category maps from a duplicate record into a master record, then purge the duplicate.</p>

                  <form onSubmit={handleMerge} className="space-y-4">
                    <div className="space-y-1">
                      <label className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">Duplicate (Source) Company ID *</label>
                      <input
                        type="text"
                        required
                        value={mergeSourceId}
                    ...
                    <div className="space-y-1 flex items-center justify-center text-slate-400 py-1">
                      <ArrowRight className="w-6 h-6 rotate-90 sm:rotate-0" />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">Master (Target) Company ID *</label>
                      <input
                        type="text"
                        required
                        value={mergeTargetId}
                    ...
                    <div className="flex gap-3 pt-2">
                      ...
                      <button
                        type="submit"
                        className="flex-1 py-3 rounded-xl bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold cursor-pointer"
                      >
                        Run Merge Execution
                      </button>
                    </div>
            ...
            {showBulkModal && (
              <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <motion.div
                  ...
                  className="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl border border-slate-200 dark:border-slate-800 space-y-6 text-slate-900 dark:text-slate-100"
                >
                  <h2 className="text-lg font-black text-slate-900 dark:text-white">Bulk Re-Classify Categories ({selectedIds.length})</h2>
                  <form onSubmit={handleBulkAssign} className="space-y-4">
                    <div className="space-y-1">
                      <label className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">Target Bank *</label>
                      <select
                        value={bulkBankId}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1">
                        <label className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">Category *</label>
                        <select
                          value={bulkCategory}
                      ...
                      <div className="space-y-1">
                        <label className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">Classification Status *</label>
                        <select
                          value={bulkStatus}
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">Internal Remarks</label>
                      <input
                        type="text"
                        value={bulkRemarks}
                    ...
                    <div className="flex gap-3 pt-2">
                      ...
                      <button
                        type="submit"
                        className="flex-1 py-3 rounded-xl bg-amber-500 hover:bg-amber-600 text-slate-950 text-xs font-bold cursor-pointer transition-colors"
                      >
