        ...
        <main className="flex-1 p-8 space-y-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-900 pb-5">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-2xl font-black text-white tracking-tight">Pincode Registry</h1>
              </div>
              <p className="text-xs text-slate-400 font-semibold mt-0.5">Manage bank coverage grids, serviceable zones, and risk-restricted areas</p>
            </div>
          ...
          {loading ? (
            <div className="py-32 text-center space-y-4">
              ...
              <p className="text-xs font-bold text-slate-400">Loading pincodes registry...</p>
            </div>
          ) : (
                      ...
                      pincodes.map((p) => (
                        <tr key={p.id} className="hover:bg-slate-850/40 transition-colors">
                          ...
                          <td className="py-4.5 px-6">
                            <div className="font-extrabold text-slate-300">{p.area || "—"}</div>
                            <div className="text-[9px] font-mono text-slate-500 font-extrabold uppercase">
                              {p.city || "—"}, {p.state || "—"}
                            </div>
                          </td>
                          <td className="py-4.5 px-6">
                            <span className="text-[10px] font-mono font-extrabold text-slate-350 bg-slate-950 px-2 py-1 rounded border border-slate-850">
                          ...
                          <td className="py-4.5 px-6">
                            <div className="flex flex-col space-y-1">
                              ...
                              {p.isNegative && (
                                <span className="px-2 py-0.5 rounded text-[8px] font-black bg-amber-500/10 border border-amber-500/20 text-amber-400 w-max flex items-center gap-1">
          ...
          <AnimatePresence>
            {showModal && (
              <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                ...
                >
                  <h2 className="text-lg font-black text-white">{editingPincode ? "Modify Pincode Serviceability" : "Map Pincode Serviceability"}</h2>
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      {/* Bank */}
                      <div className="space-y-1">
                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider">Mapped Bank *</label>
                        <select
                          value={bankId}
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      ...
                      {/* Area */}
                      <div className="space-y-1">
                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider">Area / Circle</label>
                        <input
                          type="text"
                          value={area}
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-2">
                      <div className="flex items-center space-x-2">
                        ...
                        />
                        <label htmlFor="isServiceable" className="text-xs font-bold text-slate-350 cursor-pointer">
                          Serviceable Area
                        </label>
                      </div>
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        />
                        <label htmlFor="isNegative" className="text-xs font-bold text-slate-350 cursor-pointer text-rose-400">
                          Negative List Area
                        </label>
                      </div>
                    </div>
                    ...
                    {/* Category */}
                    <div className="space-y-1">
                      <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider">Pincode Category Classification</label>
                      <select
                        value={category}
