    ...
    <main className="p-4 sm:p-8 space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 dark:border-slate-900 pb-5">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Security &amp; Integration Keys</h1>
          </div>
          <p className="text-xs text-slate-600 dark:text-slate-400 font-semibold mt-0.5">Manage machine authentication tokens, active operator sessions, and system audit logs</p>
        </div>
        <button
          onClick={() => {
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
          {/* API Keys Table */}
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-2xl space-y-4">
            <div className="flex items-center space-x-2 border-b border-slate-100 dark:border-slate-850 pb-3">
              <Key className="w-4 h-4 text-purple-600 dark:text-purple-400" />
              <h3 className="text-xs font-black uppercase text-slate-900 dark:text-white tracking-wider">Integration API Tokens</h3>
            </div>
            ...
            {apiKeys.length === 0 ? (
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
                {apiKeys.map((key) => (
                  <div key={key.id} className="p-4 bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-850 space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-extrabold text-slate-900 dark:text-white">{key.name}</span>
                    ...
                    <div className="flex items-center justify-between font-mono text-[10px] text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-900 p-2 rounded-lg border border-slate-200 dark:border-slate-800">
                      <span>{key.key.slice(0, 16)}••••••••</span>
                      {key.isActive && (
                        <button
                          onClick={() => handleRevokeApiKey(key.id)}
                          className="text-rose-600 dark:text-rose-400 hover:text-rose-700 dark:hover:text-rose-300 font-sans text-[9px] font-bold cursor-pointer"
                        >
                          Revoke
                        </button>
                      )}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-2xl space-y-4">
            <div className="flex items-center space-x-2 border-b border-slate-100 dark:border-slate-850 pb-3">
              ...
              <h3 className="text-xs font-black uppercase text-slate-900 dark:text-white tracking-wider">System Audit Trail Stream</h3>
            </div>

            <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
              {auditLogs.map((log) => (
                <div key={log.id} className="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-850 text-xs font-medium space-y-1">
                  <div className="flex items-center justify-between">
      ...
      <AnimatePresence>
        {showKeyModal && (
          <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            ...
            >
              <h2 className="text-lg font-black text-slate-900 dark:text-white">Generate Integration API Token</h2>
              ...
              {newGeneratedKey ? (
                <div className="space-y-4">
                  ...
                  <div className="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-850 font-mono text-xs text-slate-900 dark:text-white flex items-center justify-between break-all">
                    <span>{newGeneratedKey}</span>
                    <button
                      onClick={() => copyToClipboard(newGeneratedKey)}
                      className="ml-2 p-1 text-slate-500 hover:text-slate-900 dark:hover:text-white cursor-pointer shrink-0"
                    >
                ...
                <form onSubmit={handleCreateApiKey} className="space-y-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">Token Name / Integration Purpose *</label>
                    <input
                      type="text"
                      required
                      value={keyName}
