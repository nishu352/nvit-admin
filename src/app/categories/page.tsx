      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 dark:border-slate-900 pb-5">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Category Mappings</h1>
          </div>
          <p className="text-xs text-slate-600 dark:text-slate-400 font-semibold mt-0.5">Manage bank-specific category mapping tables for salaries and approvals</p>
        </div>
        <button
            ...
            <table className="w-full text-left border-collapse min-w-[650px]">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-850 text-xs font-bold text-slate-700 dark:text-slate-200">
                {mappings.length === 0 ? (
                  ...
                  mappings.map((m: any) => (
                    <tr key={m.id} className="hover:bg-slate-50 dark:hover:bg-slate-850/40 transition-colors">
                      <td className="py-4.5 px-6 font-black text-slate-900 dark:text-white">
                        <div>{m.company.name}</div>
                        <div className="text-[9px] font-mono text-slate-500 font-extrabold">{m.company.cin || "UNLISTED"}</div>
                      </td>
                      <td className="py-4.5 px-6">
                        <div className="font-extrabold text-slate-800 dark:text-slate-350">{m.bank.name}</div>
                      ...
                      <td className="py-4.5 px-6">
                        <div className="flex items-center space-x-1.5">
                          ...
                          <span className={m.status === "APPROVED" ? "text-emerald-600 dark:text-emerald-400" : m.status === "BLOCKED" ? "text-rose-600 dark:text-rose-400" : "text-amber-600 dark:text-amber-400"}>
                            {m.status}
                          </span>
                        </div>
                      </td>
                      <td className="py-4.5 px-6 font-semibold text-slate-600 dark:text-slate-400 max-w-xs truncate" title={m.remarks}>
                        {m.remarks || "—"}
                      </td>
      ...
      <AnimatePresence>
        {showModal && (
          <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            ...
            >
              <h2 className="text-lg font-black text-slate-900 dark:text-white">{editingMapping ? "Edit Classification Mapping" : "Map Institutional Classification"}</h2>
              <form onSubmit={handleSubmit} className="space-y-4">
                {/* Company */}
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">Select Company *</label>
                  <select
                    disabled={!!editingMapping}
                    value={companyId}
                    onChange={(e) => setCompanyId(e.target.value)}
                    className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 text-slate-900 dark:text-white disabled:opacity-50 rounded-xl text-xs font-semibold focus:outline-none"
                  >
                    {companies.map((c) => (
                ...
                {/* Bank */}
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">Lender Institution *</label>
                  <select
                    disabled={!!editingMapping}
                    value={bankId}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">Category Category *</label>
                    <select
                      value={category}
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">Approval Status *</label>
                    <select
                      value={status}
                ...
                {/* Remarks */}
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">Classification Remarks</label>
                  <input
                    type="text"
                    value={remarks}
