          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-900 pb-5">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-2xl font-black text-white tracking-tight">CRM Lead Management Board</h1>
              </div>
              <p className="text-xs text-slate-400 font-semibold mt-0.5">Track applicant inquiries, update underwriting statuses, and record notes</p>
            </div>
            ...
            <div className="relative w-72">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 bg-slate-900 border border-slate-850 hover:border-slate-800 rounded-xl text-xs font-semibold text-white placeholder-slate-500 focus:outline-none"
              />
            </div>
          </div>
                ...
                return (
                  <div key={stage.id} className="w-72 shrink-0 flex flex-col bg-slate-900/60 rounded-2xl border border-slate-850 p-4 space-y-3">
                    ...
                    <div className="flex items-center justify-between border-b border-slate-850 pb-3">
                      ...
                      <span className="px-2 py-0.5 rounded text-[9px] font-black bg-slate-950 text-slate-400 border border-slate-850">
                        {stageLeads.length}
                      </span>
                    </div>
                    ...
                    <div className="flex-1 space-y-3 overflow-y-auto max-h-[600px] pr-1">
                      {stageLeads.length === 0 ? (
                        <div className="py-12 text-center text-[10px] font-bold text-slate-600 border border-dashed border-slate-850 rounded-xl">
                          No leads in stage
                        </div>
                      ) : (
                        stageLeads.map((lead) => (
                          <div
                            ...
                            className="p-4 bg-slate-950 hover:bg-slate-900 rounded-xl border border-slate-850 hover:border-slate-700 transition-all cursor-pointer space-y-3 group shadow-xs"
                          >
                            <div className="flex items-start justify-between">
                              <div>
                                <h4 className="font-black text-white text-xs leading-none group-hover:text-blue-400 transition-colors">
                                  {lead.name}
                                </h4>
                                <p className="text-[10px] text-slate-400 font-extrabold mt-1">{lead.company}</p>
                              </div>
                            ...
                            <div className="space-y-1 text-[10px] text-slate-450 font-semibold border-t border-slate-900 pt-2">
                              <div className="flex items-center space-x-1.5">
                                <Phone className="w-3 h-3 text-slate-550 shrink-0" />
                                <span className="font-mono">{lead.mobile}</span>
                              </div>
                              <div className="flex items-center space-x-1.5">
                                <Building className="w-3 h-3 text-slate-550 shrink-0" />
                                <span>{lead.loanType} • ₹{(lead.monthlyIncome/1000)}k/mo</span>
                              </div>
                            </div>

                            <div className="flex items-center justify-between text-[9px] font-mono text-slate-550 border-t border-slate-900/60 pt-2">
                ...
                >
                  <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between border-b border-slate-850 pb-4">
                      <div>
                        <span className="text-[9px] font-black uppercase text-blue-400 tracking-wider">APPLICANT DOSSIER</span>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-4 bg-slate-950 rounded-xl border border-slate-850 space-y-1">
                        ...
                        <div className="text-base font-extrabold text-emerald-400">₹{selectedLead.loanAmount.toLocaleString("en-IN")}</div>
                      </div>
                      <div className="p-4 bg-slate-950 rounded-xl border border-slate-850 space-y-1">
                        <span className="text-[9px] font-black text-slate-500 uppercase">Monthly Income</span>
                        <div className="text-base font-extrabold text-white">₹{selectedLead.monthlyIncome.toLocaleString("en-IN")}</div>
                      </div>
                    </div>
                    ...
                    <div className="p-4 bg-slate-950 rounded-xl border border-slate-850 space-y-2 text-xs font-semibold text-slate-300">
                      <div className="flex justify-between border-b border-slate-900 pb-1.5">
                        ...
                        <span className="font-mono text-white">{selectedLead.mobile}</span>
                      </div>
                      <div className="flex justify-between border-b border-slate-900 pb-1.5">
                      ...
                      <div className="flex justify-between border-b border-slate-900 pb-1.5">
                        ...
                        <span className="text-white font-extrabold">{selectedLead.company}</span>
                      </div>
                      <div className="flex justify-between border-b border-slate-900 pb-1.5">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-blue-400 font-extrabold">{selectedLead.loanType}</span>
                      </div>
                    </div>

                    {/* Stage Transition Select */}
                    <div className="space-y-2 pt-2">
                      <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider">Update Pipeline Stage *</label>
                      <div className="flex gap-2">
                        <select
                          value={selectedLead.status}
                          onChange={(e) => handleStatusChange(selectedLead.id, e.target.value)}
                          className="flex-1 px-4 py-2.5 bg-slate-950 border border-slate-850 text-white rounded-xl text-xs font-semibold focus:outline-none"
                        >
                          {CRM_STAGES.map((s) => (
                    ...
                    <div className="space-y-3 pt-4 border-t border-slate-850">
                      ...
                      <form onSubmit={handleAddNote} className="flex gap-2">
                        <input
                          ...
                          className="flex-1 px-4 py-2 bg-slate-950 border border-slate-850 text-white text-xs font-semibold rounded-xl focus:outline-none"
                        />
                        <button
                          type="submit"
                          className="px-4 py-2 bg-purple-650 hover:bg-purple-700 text-white font-bold text-xs rounded-xl cursor-pointer"
                        >
                      ...
                      <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                        {(() => {
                          ...
                          if (notesArr.length === 0) return <p className="text-[10px] text-slate-500 italic">No notes logged yet.</p>;
                          return notesArr.map((n: any, idx: number) => (
                            <div key={idx} className="p-3 bg-slate-950 rounded-xl border border-slate-850 space-y-1 text-xs font-medium">
